...
class AlternatePayment extends CI_Controller
{
    ...
    public function initilize_payment()
    {
        $user_id=$this->input->get('user_id');
        file_put_contents(APPPATH . 'logs/Check_user.log', print_r($user_id, true), FILE_APPEND);
        ...
        $redirectUrl = 'https://ambrosiaayurved.in/AlternatePayment/insert_order?merchantOrderId=' . $merchantOrderId;
    ...
    public function callback()
    {
        ...
        if ($orderState === 'COMPLETED' || $orderState === 'FAILED') {
            ...
            $url = site_url('AlternatePayment/saved_payment_data'); // No base_url here
    ...
    public function saved_payment_data()
    {
        ...
        // Logging
        file_put_contents(APPPATH . 'logs/.saved_payment_datalog', print_r($data, true), FILE_APPEND);

        // Store in DB
        if (!empty($data)) {
            file_put_contents(APPPATH . 'logs/insert_data_debug.log', "Inserting...\n", FILE_APPEND);
            $this->AlternatePayment_model->insert_payment_data($data);
        }
    ...
    public function get_amount()
    {
        ...
        $cookie_cart = get_cookie('alternate_order');
        $cart_items = $cookie_cart ? json_decode($cookie_cart, true) : [];
  // Log cookie data
    // log_message('debug', 'Alternate Order Cookie: ' . print_r($cart_items, true));
        // file_put_contents(APPPATH . 'logs/.test_alternate_amount', print_r($cart_items, true), FILE_APPEND);
    ...
    //     $alternate_flow = get_cookie('alternate_order');
    //     // $cart_cookie = get_cookie('cart');
    //     $cart_items = [];

    //     if (!empty($alternate_flow)) {
    //         $item = json_decode($alternate_flow, true);
    ...
    public function insert_order()
    {
        ...
        file_put_contents(APPPATH . 'logs/Check_pmnt_status.log', print_r($status, true), FILE_APPEND);
            ...
            // if(!empty($user_id)&&$is_user_exist==1)
            // {

            // }
            // if(empty($user_id)&& $is_user_exist==0)
            // {
            //      $user_id = $this->AlternatePayment_model->insert_user_data('user_table', $user_data);
            // }
        // file_put_contents(APPPATH . 'logs/check_user.log', print_r($user_data, true), FILE_APPEND);
        // file_put_contents(APPPATH . 'logs/check_user_id.log', print_r($user_id, true), FILE_APPEND);
            if (!$user_id) {
                // echo "User creation failed!";
                 file_put_contents(APPPATH . 'logs/user_insertion_fail.log', "Insertion failed result: " . var_export($user_id, true), FILE_APPEND);
                return;
            }
            ...
            //     // 'created_at' => date('Y-m-d H:i:s')
            // ];
        file_put_contents(APPPATH . 'logs/address_insertion.log', print_r($address_data, true), FILE_APPEND);
        ...
        // Get cart items from alternate order cookie
        $alternate_flow = get_cookie('alternate_order');
        $cart_items = [];
        ...
        foreach ($cart_items as $item) {
            ...
            $order_data = [
                ...
                'product_id'       => $item['product_id'],
                'order_id'         => $merchantOrderId,
                'address_id'      => $address_id, // Not using address IDs anymore
                'product_price'   => $product_price,
                'product_quantity' => $quantity,
                'total_price'     => $product_price * $quantity,
                'created_at'      => date('Y-m-d H:i:s'),
