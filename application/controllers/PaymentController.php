...
class PaymentController extends CI_Controller
{
    ...
    {
        // file_put_contents(APPPATH . 'logs/.initiate_payment_with_user_id.log', print_r($user_id, true), FILE_APPEND);
        $merchantOrderId = "ORDER_" . time();
    ...
    public function callback()
    {
        ...
        file_put_contents(APPPATH . 'logs/Data_form_Flutter_app.log', print_r($payload, true), FILE_APPEND);
    ...
    public function insert_order()
    {
        ...
        $merchantOrderId = $this->input->get('merchantOrderId');
        $user_id = $this->session->userdata('user_id');
        $inserted_user_id = $this->Payment_model->insert_user_id($merchantOrderId, $user_id);
        // if(!$inserted_user_id)
        // {
        ...
        // preg_match('/ORDER_(\d+)_UID_(\d+)/', $merchantOrderId, $matches);
        // $merchantOrderId = $matches[1] ?? null;
        // $user_id = $matches[2] ?? null;
        $get_status = $this->Payment_model->get_payment_status($merchantOrderId);

        // file_put_contents(APPPATH . 'logs/Check_merchant_and_used_id.log', print_r($merchantOrderId, $user_id,  true), FILE_APPEND);
