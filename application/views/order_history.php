                                ...
                                <tr>
                                    ...
                                    <td style=" display:flex;justify-content:space-between;">
                                        ...
                                        <?php if ((int)$order->status !== 5 && (int)$order->status !== 2 && (int)$order->status !== 0): ?>
                                            <form class="cancel-order-form" method="POST">
                                                <input type="hidden" name="order_id" value="<?php echo $order->order_id; ?>">
                                                <button type="button" class="remove-btn" onclick="cancelOrder('<?php echo $order->awb_code; ?>')">Cancel</button>
                                            </form>
                                        <?php endif; ?>
...
<script>
  ...
  $.ajax({
    ...
    function cancelOrder(awb_code) {
        if (!confirm('Are you sure you want to cancel this order?')) return;

        $.ajax({
            url: '<?= base_url("Tracking/cancel_order") ?>',  
            method: 'POST',
